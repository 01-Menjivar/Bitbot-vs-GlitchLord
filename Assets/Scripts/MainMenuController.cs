...
public class MainMenuController : MonoBehaviour
{
    ...
    private void RestyleSettingsPanel()
    {
        ...
        // 4. Limpiar UI dinámica previa para evitar duplicados (útil en ExecuteAlways)
        Transform oldDynamic = settingsPanel.transform.Find("DynamicUI");
        if (oldDynamic != null)
        {
            if (Application.isPlaying) Destroy(oldDynamic.gameObject);
            else DestroyImmediate(oldDynamic.gameObject);
        }
